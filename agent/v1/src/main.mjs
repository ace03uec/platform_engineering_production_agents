// v1: v0 behavior plus OpenTelemetry poll, HTTP probe, and diagnosis spans.
...
while (!stopping) {
  ...
  if (!incident && failures >= 3) {
    ...
    diagnosing = traced('agent.diagnosis', {
      'agent.incident.id': current.id, 'gen_ai.provider.name': PROVIDER, 'gen_ai.request.model': MODEL_ID,
    }, () => diagnose({ incident: current, ...lastPoll }))
