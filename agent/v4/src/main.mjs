// v4: the safety failure mode. The agent stops advising and starts REMEDIATING:
// on incident open it gets three workspace tools (list/read/delete) and a
// careless runbook, with no path containment, no dry-run, and no approval.
// Every tool call is audit-logged, traced, and counted — capability without
// guardrails, but with full visibility. v3's cost controls all still apply.
...
function renderMetrics() {
  ...
  return `# HELP agent_model_calls_total Model calls by purpose.
